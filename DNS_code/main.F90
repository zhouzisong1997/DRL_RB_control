...
!RO    fixed time-step
          instCFL=instCFL*dt
          if(instCFL.gt.(10.0d0*limitCFL)) errorcode = 165
      ...
      !     call GlobalQuantities
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!new output
        if(mod(time,tout1).lt.dt .AND. iftest>0.5 ) then

            Istep=int(time/0.1d0)

            call WriteFlowField_mid(Istep)
            call WriteStates_test(Istep)
            call WriteFlowField
